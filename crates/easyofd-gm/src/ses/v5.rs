...
///
/// V5 与 V4 结构相同，均以 TBS_Sign 作为首元素。
///
/// ```asn1
/// SES_Signature ::= SEQUENCE {
///     toSign              TBS_Sign,
...
impl TBSSign {
    /// DER 编码。
    ///
    /// 对应 Java: `org.ofdrw.gm.ses.v5.TBS_Sign#toASN1Primitive`
    pub fn encode_der(&self) -> Vec<u8> {
        build_sequence(|inner| {
            encode_integer(self.version, inner);
    ...
    /// DER 解码。
    ///
    /// 对应 Java: `org.ofdrw.gm.ses.v5.TBS_Sign(ASN1Sequence)`
    pub fn decode_der(der: &[u8]) -> DerResult<Self> {
        let (val, _) = decode_sequence(der, 0)?;
        let mut pos = 0;
...
impl SESSignature {
    /// DER 编码。
    ///
    /// 对应 Java: `org.ofdrw.gm.ses.v5.SES_Signature#toASN1Primitive`
    pub fn encode_der(&self) -> Vec<u8> {
        build_sequence(|inner| {
    ...
    /// DER 解码。
    ///
    /// 对应 Java: `org.ofdrw.gm.ses.v5.SES_Signature(ASN1Sequence)`
    pub fn decode_der(der: &[u8]) -> DerResult<Self> {
        let (val, _) = decode_sequence(der, 0)?;
        let mut pos = 0;
