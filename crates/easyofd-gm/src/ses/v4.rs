//! SES V4 电子印章 ASN.1 结构定义。
//!
//! 对应 Java 版 [`org.ofdrw.gm.ses.v4`](https://github.com/ofdrw/ofdrw) 包。
//! V4 相较 V1 的主要变化：
//!
//! - `SESeal` 展平：cert / signatureAlgorithm / signData 提升至顶层。
//! - `SES_SealInfo` 增加 `extDatas` 可选扩展。
//! - 时间字段改用 GeneralizedTime（YYYYMMDDHHmmSSZ）。
//! - `CertList` 支持 CHOICE：完整证书（OCTET STRING）或证书摘要。
//!
//! # 结构概览
//!
//! ```asn1
...
use super::{
    ...
    decode_oid, decode_sequence, decode_tlv, decode_uint, encode_bit_string,
    encode_generalized_time, encode_ia5_string, encode_integer, encode_octet_string, encode_oid,
    encode_printable_string, encode_sequence, expect_tlv,
};
...
mod java_name_aliases {
    use super::{CertDigest, SESHeader, SESPropertyInfo, SESSignature, SealInfo, TBSSign};

    /// 别名：对应 Java `org.ofdrw.gm.ses.v4.TBS_Sign`。
    pub type TBS_Sign = TBSSign;
    /// 别名：对应 Java `org.ofdrw.gm.ses.v4.SES_Signature`。
    pub type SES_Signature = SESSignature;
    /// 别名：对应 Java `org.ofdrw.gm.ses.v4.SES_ESPropertyInfo`。
    pub type SES_ESPropertyInfo = SESPropertyInfo;
    /// 别名：对应 Java `org.ofdrw.gm.ses.v4.SES_SealInfo`。
    pub type SES_SealInfo = SealInfo;
    /// 别名：对应 Java `org.ofdrw.gm.ses.v4.SES_Header`。
    pub type SES_Header = SESHeader;
    /// 别名：对应 Java `org.ofdrw.gm.ses.v4.CertDigestObj`。
...
/// V4 印章头信息。
///
/// 对应 Java: `org.ofdrw.gm.ses.v4.SES_Header`
...
/// V4 证书摘要信息（CertList CHOICE 之一）。
///
/// 对应 Java: `org.ofdrw.gm.ses.v4.CertDigest`
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// V4 证书列表项。
///
/// 支持 CHOICE：完整证书（`OCTET STRING`）或证书摘要（`CertDigest`）。
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum CertChoice {
    /// 完整证书（DER 编码）。
    FullCert(Vec<u8>),
...
/// V4 印章属性信息。
///
/// 对应 Java: `org.ofdrw.gm.ses.v4.SES_ESPropertyInfo`
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SESPropertyInfo {
    ...
    /// 创建日期，GeneralizedTime 格式 "YYYYMMDDHHmmSSZ"。
...
/// V4 印章图片信息。
///
/// 对应 Java: `org.ofdrw.gm.ses.v4.SES_ESPictureInfo`
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// V4 印章信息。
///
/// 对应 Java: `org.ofdrw.gm.ses.v4.SES_SealInfo`
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SealInfo {
    /// 印章头。
...
/// V4 电子印章。
///
/// V4 展平结构：cert / signatureAlgorithm / signData 直接在顶层。
///
/// 对应 Java: `org.ofdrw.gm.ses.v4.SESeal`
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SESeal {
    /// 印章信息。
...
mod tests {
    ...
    const SM3_OID: &[u32] = &[1, 2, 156, 10_197, 1, 401];
    ...
    fn sample_property_full_cert() -> SESPropertyInfo {
        SESPropertyInfo {
            seal_type: 0,
            name: "V4TestSeal".into(),
    ...
    fn sample_property_with_digest() -> SESPropertyInfo {
        SESPropertyInfo {
            seal_type: 1,
            name: "V4DigestSeal".into(),
            cert_list: vec![
                CertChoice::FullCert(vec![0x30, 0x03, 0x02, 0x01, 0x01]),
                CertChoice::Digest(CertDigest {
                    cert: vec![0x30, 0x03, 0x02, 0x01, 0x02],
                    digest_alg: SM3_OID.to_vec(),
    ...
    fn sample_seal_info() -> SealInfo {
        SealInfo {
            ...
            es_id: "ES_V4_001".into(),
            property: sample_property_full_cert(),
    ...
    fn seseal_flat_fields_preserved() {
        ...
        assert_eq!(decoded.sign_data, vec![0xBB; 64]);
        // 内部印章信息
        assert_eq!(decoded.eseal_info.header.id, "ES");
        assert_eq!(decoded.eseal_info.header.version, 4);
        assert_eq!(decoded.eseal_info.es_id, "ES_V4_001");
        assert_eq!(decoded.eseal_info.picture.width, 300);
    }
    ...
    #[test]
    fn property_with_cert_digest_roundtrip() {
        let prop = sample_property_with_digest();
        let der = prop.encode_der();
        let decoded = SESPropertyInfo::decode_der(&der).unwrap();
        assert_eq!(decoded.cert_list.len(), 2);
        assert!(matches!(decoded.cert_list[0], CertChoice::FullCert(_)));
        assert!(matches!(decoded.cert_list[1], CertChoice::Digest(_)));
        if let CertChoice::Digest(ref d) = decoded.cert_list[1] {
            assert_eq!(d.digest_alg, SM3_OID);
            assert_eq!(d.digest_value, vec![0xAB; 32]);
        }
    }
    ...
    #[test]
    fn generalized_time_format_in_property() {
        let prop = sample_property_full_cert();
        let der = prop.encode_der();
        let decoded = SESPropertyInfo::decode_der(&der).unwrap();
        assert_eq!(decoded.create_date, "20250101000000Z");
        assert_eq!(decoded.valid_start, "20250101000000Z");
        assert_eq!(decoded.valid_end, "20300101000000Z");
    }

    #[test]
    fn empty_sign_data_roundtrip() {
        let mut seal = sample_seseal();
